...
class Transformer(nn.Module):
    def __init__(self, inputsize,embedding_dim,hidden_dim,out_size,batch_size=1,n_layer = 1, dropout = 0,
                 embedding = None):
        super(Transformer, self).__init__()    #   这里的5是指输入的特征数
        ...
        #                   num_layers=self.n_layer, bidirectional=False)
        self.transformer = nn.Transformer(5, hidden_dim, n_layer)
        ...
        self.out = nn.Linear(5, hidden_dim)
    ...
    def forward(self, x):
        ...
        output0 = self.transformer(input,input)
        # print(0)
        ...
        # hn0 = torch.squeeze(output0,dim=2)
        output = self.out(output0)
