...
class GRU(nn.Module):
    def __init__(self, inputsize,embedding_dim,hidden_dim,out_size,batch_size=1,n_layer = 1, dropout = 0,
                 embedding = None):
        super(GRU, self).__init__()    #   这里的5是指输入的特征数
        ...
        self.firstLayer = nn.Sequential(nn.Linear(inputsize, 3),  # Loan_application_Configuration4
                                        # nn.ReLU(),
                                        # nn.Linear(45, 30),
                                        # nn.ReLU(),
                                        # nn.Linear(30, 5),
    ...
    def forward(self, x):
        ...
        # print(0)
        # s = torch.load("myfinalHideState5weihelpdesk1.pth")
        # q = final_hidden_state[-1].detach().numpy().reshape(9, 10)
        # temp = np.vstack((s, q))
        #
        # torch.save(q, "myfinalHideState5weihelpdesk1.pth")
        # torch.save(temp, "myfinalHideState5weihelpdesk1.pth")
        hn0 = output0[-1]
        output = self.out(hn0)
