...
def train(data_address,data_address1,data_address2, data_name,output_dir,latest_checkpoint_file,start_record_epoch,vector_address=None, embd_dimension=2, embd_dimension_suffix=5, hidden_dim_suffix=2,
    ...
    print("SplitData Finish")
    data2 = InputData(data_address2, embd_dimension=embd_dimension)
    print("InputData Finish")
    ...
    print("EncodeTrace Finish")
    # 通过设置固定的随机数种子以获取相同的训练集和测试集,timeEmbedding=data.timeEmbedding
    data2.splitData(train_splitThreshold)
    print("SplitData Finish")
    ...
    train_singlePrefixData1_2, train_labelPrefixData_2, test_singlePrefixData1_2, test_labelPrefixData_2, \
                train_singlePrefixData_processing_2, test_singlePrefixData_processing_2 = data2.initBatchData_Prefix()
    ...
    if model_type == 'GRU':
        ...
        model = DPCNN(len(train_singlePrefixData1[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                      out_size=out_size,
                      batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
        # print("len(train_singlePrefixData1[0]):    $$$$$$$$$", len(train_singlePrefixData1[0]))
    if model_type == 'GRU':
        model2 = GRU(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                     out_size=out_size,
                     batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'GRU_new':
        model2 = GRU_new(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                         out_size=out_size,
                         batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'RNN':
        model2 = RNN(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                     out_size=out_size,
                     batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'LSTM':
        model2 = LSTM(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                      out_size=out_size,
                      batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'CNN':
        model2 = CNN(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                     out_size=out_size,
                     batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'TF':
        model2 = Transformer(len(train_singlePrefixData1_2[0]), nhead=1, dim_feedforward=hidden_dim,
                             num_layers=n_layer, dropout=dropout)
    elif model_type == 'TCNN':
        model2 = TextCNN(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                         out_size=out_size,
                         batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'TRCNN':
        model2 = TextRCNN(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                          out_size=out_size,
                          batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'FT':
        model2 = FastText(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                          out_size=out_size,
                          batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    elif model_type == 'DPCNN':
        model2 = DPCNN(len(train_singlePrefixData1_2[0]), embedding_dim=embd_dimension, hidden_dim=hidden_dim,
                       out_size=out_size,
                       batch_size=batch_size, n_layer=n_layer, dropout=dropout, embedding=data.embedding)
    ...
    optimizer2 = optim.Adam(model2.parameters(), lr=learn_rate)
    ...
    tensor_x_2 = torch.from_numpy(np.array(train_singlePrefixData1_2).astype(np.float32))
    tensor_y_2 = torch.from_numpy(np.array(train_labelPrefixData_2).astype(np.float32))
    my_dataset_2 = torch.utils.data.TensorDataset(tensor_x_2, tensor_y_2)
    my_dataset_loader_2 = DataLoader(my_dataset_2, batch_size, drop_last=True, shuffle=False)

    tensor1_x_2 = torch.from_numpy(np.array(test_singlePrefixData1_2).astype(np.float32))
    tensor1_y_2 = torch.from_numpy(np.array(test_labelPrefixData_2).astype(np.float32))
    testPre_dataset_2 = torch.utils.data.TensorDataset(tensor1_x_2, tensor1_y_2)
    testPre_dataset_loader_2 = DataLoader(testPre_dataset_2, batch_size, drop_last=True, shuffle=False)
    out_txt_path='./result_baseline/TF_test_metric_result_5_50%zi.txt'
    out_csv_path = './result_baseline/TF_test_metric_result_5_50%zi.csv'
    ...
    for enpoch in range(100):
        ...
        total_loss_2 = torch.FloatTensor([0])
        ...
        for i_2, (input_2, target_2) in enumerate(my_dataset_loader_2):
            optimizer2.zero_grad()
            # print("input:              ", np.array(input).shape)

            output_2 = model2(Variable(input_2))
            loss_2 = criterion(output_2, target_2)
            # optimizer.zero_grad()
            loss_2.backward(retain_graph=True)
            optimizer2.step()
            total_loss_2 += loss_2.data
        ACC = 0
        F1_EVERY_CLASS = 0
        F1_MACRO = 0
        F1_MICRO = 0
        ...
        predList_3=[]
        realList_3=[]
        for j, (input_zip1, input_zip2,input_zip3) in enumerate(zip(testPre_dataset_loader,testPre_dataset_loader_1,testPre_dataset_loader_2)):
            # print("input:              ", np.array(input).shape)
            times=0
            input1=list(input_zip1)
            input2=list(input_zip2)
            input3 = list(input_zip3)
            ...
            for l2 in range(len(predList_2_2)):
                ...
                if predList_2_2[l2] >= 0.5:
                    predList_2_2[l2] = 1
            input1_3 = input3[0].float()
            target1_3 = input3[1].float()
            start_time3 = time.time()
            output_3 = model2(input1_3)
            end_time3 = time.time()
            time3 = end_time3 - start_time3
            time_list=[time1,time2,time3]
            min_time=min(time_list)
            max_time = max(time_list)
            output1_3 = output_3.detach().numpy()
            predList_3 += [pdic1_3.item() for pdic1_3 in output1_3]
            realList_3 += [pdic2_3.item() for pdic2_3 in target1_3]
            predList_3_3 = copy.deepcopy(predList_3)
            for line3 in range(len(output1_3)):
                if output1_3[line3][0] < 0.5:
                    output1_3[line3][0] = 0
                if output1_3[line3][0] >= 0.5:
                    output1_3[line3][0] = 1
            for l3 in range(len(predList_3_3)):
                if predList_3_3[l3] < 0.5:
                    predList_3_3[l3] = 0
                if predList_3_3[l3] >= 0.5:
                    predList_3_3[l3] = 1

            output_final=output1_3
            for line4 in range(len(output_final)):
                if output1_1[line4][0] == 1 and output1_2[line4][0] == 1 and output1_3[line4][0] == 1:
                    output_final[line4][0] = 1
                else:
                    output_final[line4][0] = 0

            target1_1 = target1_1.detach().numpy()
            target1_2 = target1_2.detach().numpy()
            target1_3 = target1_3.detach().numpy()
            target_final = target1_3
            for line5 in range(len(target_final)):
                if target1_1[line5][0] == 1 and target1_2[line5][0] == 1 and target1_3[line5][0] == 1:
                    target_final[line5][0] = 1
                else:
                    target_final[line5][0] = 0
            for i in range(len(predList_1_1)):
                if predList_1_1[i] ==1 and predList_2_2[i]==1 and predList_3_3[i]==1:
...
if __name__ == '__main__':
    ...
    train(data_address='E:/PycharmProjects/Article2/Data/output_second_zi/train_result_5_1_50%.csv',  # 修改数据名
          data_address1='E:/PycharmProjects/Article2/Data/output_second_zi/train_result_5_2_50%.csv',
          data_address2='E:/PycharmProjects/Article2/Data/output_second_zi/train_result_5_3_50%.csv',
          ...
          train_type='iteration',output_dir='./model_baseline',latest_checkpoint_file='TF_test_result5_50%zi',start_record_epoch=0)
